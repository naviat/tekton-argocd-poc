...
echo " _____ ___  ____ _____ _   _    _     "
echo "|  ___/ _ \|  _ \_   _| \ | |  / \    "
echo "| |_ | | | | |_) || | |  \| | / _ \   "
echo "|  _|| |_| |  _ < | | | |\  |/ ___ \  "
...
initK8SResources() {
  ...
  while [[ $(kubectl get pods -l 'app in (nexus)' --all-namespaces -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]]; do echo "waiting for nexus pods ready..." && sleep 10; done
  while [[ $(kubectl get pods -l 'app in (sonarqube)' --all-namespaces -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]]; do echo "waiting for sonarqube pods ready..." && sleep 10; done
  while [[ $(kubectl get pods -l 'app in (tekton-pipelines-controller)' --all-namespaces -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]]; do echo "waiting for tekton-pipelines-controller pods ready..." && sleep 10; done
  while [[ $(kubectl get pods -l 'app in (tekton-dashboard)' --all-namespaces -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]]; do echo "waiting for tekton-dashboard pods ready..." && sleep 10; done
}
